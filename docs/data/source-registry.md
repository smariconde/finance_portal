...
| ------------------------- | ------------------------------------------------------- | ---- | -------------------- | ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `sec-edgar`               | identidad CIK, submissions, filings y XBRL/companyfacts | 2    | `integrated`         | `approved_personal`     | Universo, companyfacts, verificación de sucesiones de emisor, ratios de split de `companyconcept` y evidencia de traspasos, delistings y renombres en `submissions` integrados detrás del gate de derechos y del egress; User-Agent con contacto real y 2 req/s ([ADR 0010](../architecture/adr/0010-sec-xbrl-ingestion.md), [ADR 0011](../architecture/adr/0011-issuer-succession-reporting-lineage.md), [ADR 0012](../architecture/adr/0012-stock-splits-share-basis.md), [ADR 0013](../architecture/adr/0013-listing-events-dated-evidence.md)); companyfacts se guarda en la ventana de cinco ejercicios de la [ADR 0017](../architecture/adr/0017-sec-history-window.md); `rawStorage`, `publicDisplay` y `export` pasaron a `allowed` el 2026-09-18 para congelar extractos reales como oráculo de regresión, y `aiTransfer` sigue sin revisar a propósito ([ADR 0023](../architecture/adr/0023-frozen-sec-extracts-rights.md)) |
| `comafi-cedear`           | registro CEDEAR: programas, ISIN, código CV y ratio     | 7    | `integrated`         | `approved_personal`     | emisor de 364 programas; sus términos **prohíben** almacenar y el owner decidió proceder con la cláusula citada, derechos en `owner_accepted` ([ADR 0027](../architecture/adr/0027-cedear-registry-sources.md)); JSON de la tabla de programas, no la planilla                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `caja-valores-cedear`     | registro CEDEAR: programas, ISIN, código CV y ratio     | 7    | `integrated`         | `approved_personal`     | segundo emisor autorizado por la CNV; sin términos publicados, derechos en `owner_accepted` ([ADR 0027](../architecture/adr/0027-cedear-registry-sources.md)); tabla HTML de la página de CEDEAR, una request por corrida                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
Hay **dos emisores** de CEDEAR autorizados por la CNV, Banco Comafi y Caja de
Valores, y el registro lee a los dos (`F7-03`,
[ADR 0027](../architecture/adr/0027-cedear-registry-sources.md)). Ninguna de las
dos publicaciones es un historial ni fecha lo que publica: el registro toma la
observación como vigencia, supersede lo que cambia y retira lo que el emisor deja
de listar. Comafi publica el mismo registro dos veces —el JSON que alimenta su
tabla y una planilla descargable— y no coinciden; la publicación de registro es el
JSON, por la razón medida que da la ADR.
...
### Extractos reales congelados

Una fixture sintética prueba que el parser hace lo que creemos; no prueba que el
cable sea como creemos. Esa segunda mitad la cubre un **corpus congelado**, y la
[ADR 0023](../architecture/adr/0023-frozen-sec-extracts-rights.md) lo autoriza por
ahora sólo para `sec-edgar`. Un corpus no es un recording, y se distingue por
cuatro propiedades que tienen que valer a la vez:

1. es un extracto **elegido** de la fuente, no lo que pasó por una corrida del
   modo personal;
2. está **reducido** por una versión declarada del reductor, que dice qué tiró;
3. viene con **manifiesto**: URL, instante de descarga, bytes y `sha256`;
4. la fuente tiene sus derechos revisados en `allowed` para conservar y
   redistribuir.

Congelar no es cachear: el corpus se descarga una vez, entra por un diff revisable
y no vuelve a la red. Los derechos son por fuente: que una parezca abierta no
alcanza —`datahub-sp500-pddl` declara PDDL y sigue en `rights_review_pending`—.
...
- Fecha efectiva de los cambios de ratio CEDEAR: los emisores la publican en avisos
  a tenedores que el registro todavía no lee; hasta entonces un cambio se supersede
  al observarlo (ADR 0027).
