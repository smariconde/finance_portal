...
| `TM-10` | Abuso o loop agota cuota, crédito IA o recursos de Function. Denial of service.                                          | high      | `contracted`: páginas sin llamadas live, batching y presupuesto por corrida. `implemented` para la SEC: espaciado, concurrencia 1, presupuesto de requests por corrida que corta antes de abrir la conexión y techo de archivos históricos por empresa.                                                                                                                                                                                                                                                                                                 | Proveedor/IA no se habilita sin límite por request/día, circuit breaker, timeout, observabilidad y kill switch. El backfill reserva el peor caso de una empresa (66 requests) antes de empezarla y una señal de la fuente frena el job (ADR 0015). Falta el límite diario y el kill switch por fuente: segundo incremento de `F2-05`. |
| `TM-11` | Cron o refresh se repite, corre en paralelo o procesa un mensaje venenoso. Tampering / denial of service.                | high      | `implemented` para el backfill de la SEC ([ADR 0015](../architecture/adr/0015-durable-ingestion-jobs.md)): job idempotente por plan, lease por fuente con token que cerca cada escritura y vencimiento que habilita la toma, cursor recalculado en el mismo commit que el checkpoint, intento contado al empezar y envenenado en el techo, `429`/`403`/`503` y red caída difieren el job sin gastar intentos, recuperación manual auditada. Probado con concurrencia real entre conexiones y con `kill -9` sobre la SEC. Cron live sigue deshabilitado. | Falta el kill switch por fuente y el límite diario antes de programar un refresh. Los comandos manuales de un ticker no toman el lease. Un intento en curso no se aborta al perder el lease: termina y su checkpoint se rechaza.                                                                                                      |
