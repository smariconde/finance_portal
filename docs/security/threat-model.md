...
| `TM-11` | Cron o refresh se repite, corre en paralelo o procesa un mensaje venenoso. Tampering / denial of service.                | high      | `implemented` para el backfill de la SEC ([ADR 0015](../architecture/adr/0015-durable-ingestion-jobs.md)): job idempotente por plan, lease por fuente con token que cerca cada escritura y vencimiento que habilita la toma, cursor recalculado en el mismo commit que el checkpoint, intento contado al empezar y envenenado en el techo, `429`/`403`/`503` y red caída difieren el job sin gastar intentos, recuperación manual auditada. Probado con concurrencia real entre conexiones y con `kill -9` sobre la SEC. Cron live sigue deshabilitado. | El kill switch y el límite diario ya están (ADR 0020) y los comandos manuales de un ticker los respetan, aunque sigan sin tomar el lease: lo que comparten es el contador de la fuente, no el turno del job. Un intento en curso no se aborta al perder el lease: termina y su checkpoint se rechaza. Cron live sigue deshabilitado.                                  |
