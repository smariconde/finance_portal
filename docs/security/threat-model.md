...
| `TM-05` | Parser roto, fuente comprometida o respuesta parcial publica datos falsos. Tampering.                                    | critical  | `implemented` para la SEC: envelope estricto por documento que cuarentena sin tocar lo publicado, filas inválidas rechazadas por campo y nunca por valor, importes exactos desde el texto fuente, disponibilidad desde la aceptación, y lo que no tiene desempate rechazado con nombre ([ADR 0010](../architecture/adr/0010-sec-xbrl-ingestion.md)). `contracted` para el resto de las fuentes.                                                                                                                                                                                                                                                                                                                                                                                                                                          | Falta la reconciliación contra el filing de 30 empresas de arquetipos distintos y la validación semántica XBRL de muestra (Arelle/DQC), que son el gate de Fase 2. Una falla sigue poniendo en cuarentena, nunca reemplaza por vacío o cero.                                                                                                                          |
...
| `TM-16` | Logs insuficientes impiden explicar una mutación, ingesta o resultado incorrecto. Repudiation.                           | medium    | `implemented` para ingesta, valuación y jobs: corridas append-only con hash y versión, y bitácora append-only de jobs con orden total, holder, token, decisión de cada intento y motivo redactado de cada acción manual. `contracted`: IDs de request para fronteras web.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | El slice que cree la operación añade eventos y failure-path tests; no se guardan prompts completos, headers ni documentos privados.                                                                                                                                                                                                                                   |
