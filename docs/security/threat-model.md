...
- Revisado el 2026-09-05: la ADR 0009 abrió la primera salida a red y cerró `TM-08` en su parte de red; `B5` deja de ser una frontera contratada
- Revisado el 2026-09-14: la ADR 0010 publicó los primeros datos reales de la SEC; `TM-05` pasa a implementado para esa fuente y el ritmo de `TM-08`/`TM-10` llega con el provider
- Revisado el 2026-09-16: la ADR 0015 agregó jobs durables de ingesta; `TM-11` pasa a implementado para el backfill de la SEC y `B4` deja de ser sólo contrato
...
| `B4` runtime -> jobs       | replay, concurrencia, costo y fallas parciales       | schema, presupuesto por corrida con reserva del peor caso, idempotencia por plan y por contenido, lease por fuente con token y vencimiento, poison policy y último snapshot válido ([ADR 0015](../architecture/adr/0015-durable-ingestion-jobs.md)) |
| `B5` jobs -> terceros      | SSRF, schema hostil, prompt injection y términos     | una única puerta (`getEgressClient()`): allowlist por fuente, resolución fijada al socket, redirects re-autorizados, techo de bytes y deadline; más validación, provenance y matriz de uso aprobada                                                 |
...
| `TM-05` | Parser roto, fuente comprometida o respuesta parcial publica datos falsos. Tampering.                                    | critical  | `implemented` para la SEC: envelope estricto por documento que cuarentena sin tocar lo publicado, filas inválidas rechazadas por campo y nunca por valor, importes exactos desde el texto fuente, disponibilidad desde la aceptación, y lo que no tiene desempate rechazado con nombre ([ADR 0010](../architecture/adr/0010-sec-xbrl-ingestion.md)). Desde `F2-06` el oráculo de regresión deja de ser sólo sintético: un corpus congelado de extractos reales, fijado por `sha256` y reconciliado contra el filing, hace que un cambio del cable o del parser rompa un test con nombre en vez de aparecer como una observación rara en el modo personal ([ADR 0023](../architecture/adr/0023-frozen-sec-extracts-rights.md)). `contracted` para el resto de las fuentes.                                                                | Falta la reconciliación contra el filing de 30 empresas de arquetipos distintos y la validación semántica XBRL de muestra (Arelle/DQC), que son el gate de Fase 2; el corpus de `F2-06` son seis filers y es la infraestructura que esa reconciliación va a usar, no la reconciliación. Una falla sigue poniendo en cuarentena, nunca reemplaza por vacío o cero.     |
...
| `TM-08` | Una URL, redirect o DNS controlado alcanza metadata, localhost o red privada. SSRF.                                      | high      | `implemented` para la red: allowlist por fuente que empareja host y prefijo de path, HTTPS forzado, resolución validada y fijada al socket, cada redirect re-autorizado, rangos privados y metadata bloqueados, techo de bytes y deadline ([ADR 0009](../architecture/adr/0009-egress-boundary.md)).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | El ritmo por corrida —2 requests/s, de a una y 1.000 por corrida— llegó con el provider de la SEC en `F2-03` (`egress-fetch.ts`). Lease, reanudación y exclusión entre procesos llegaron con la ADR 0015; la ingesta sigue siendo un job manual y no un refresh programado.                                                                                           |
...
| `TM-10` | Abuso o loop agota cuota, crédito IA o recursos de Function. Denial of service.                                          | high      | `contracted`: páginas sin llamadas live y batching. `implemented` para la SEC y el archivo de constituyentes: espaciado, concurrencia 1, presupuesto por corrida que corta antes de abrir la conexión, techo de archivos históricos por empresa, y **límite diario por fuente en PostgreSQL más kill switch del owner**, los dos aplicados en la única puerta de salida y compartidos por todos los procesos ([ADR 0020](../architecture/adr/0020-source-daily-budget-kill-switch.md)).                                                                                                                                                                                                                                                                                                                                                  | Una fuente sin tope declarado no emite ninguna llamada, y el tope guardado sólo puede bajar el declarado en código. El backfill reserva el peor caso de una empresa (66 requests) contra la corrida **y** contra el día antes de empezarla, y una señal de la fuente frena el job (ADR 0015). Falta para IA: límite por request, breaker y observabilidad de crédito. |
| `TM-11` | Cron o refresh se repite, corre en paralelo o procesa un mensaje venenoso. Tampering / denial of service.                | high      | `implemented` para el backfill y el refresh de la SEC ([ADR 0015](../architecture/adr/0015-durable-ingestion-jobs.md), [ADR 0022](../architecture/adr/0022-companyfacts-refresh-probe.md)): job idempotente por plan, lease por fuente con token que cerca cada escritura y vencimiento que habilita la toma, cursor recalculado en el mismo commit que el checkpoint, intento contado al empezar y envenenado en el techo, `429`/`403`/`503` y red caída difieren el job sin gastar intentos, recuperación manual auditada. Probado con concurrencia real entre conexiones y con `kill -9` sobre la SEC, sobre los dos kinds de job. El refresh agrega su propia idempotencia: lo que el sondeo vio entra a la clave, así que una vuelta sin novedades queda `duplicate` y no publica una corrida nueva. Cron live sigue deshabilitado. | El kill switch y el límite diario ya están (ADR 0020) y los comandos manuales de un ticker los respetan, aunque sigan sin tomar el lease: lo que comparten es el contador de la fuente, no el turno del job. Un intento en curso no se aborta al perder el lease: termina y su checkpoint se rechaza. Cron live sigue deshabilitado.                                  |
...
| `TM-16` | Logs insuficientes impiden explicar una mutación, ingesta o resultado incorrecto. Repudiation.                           | medium    | `implemented` para ingesta, valuación y jobs: corridas append-only con hash y versión, y bitácora append-only de jobs con orden total, holder, token, decisión de cada intento y motivo redactado de cada acción manual. El corpus congelado de `F2-06` lleva su propio manifiesto: de qué URL salió cada archivo, en qué instante, cuánto pesaba el documento entero que no se conserva y qué hash tenía. `contracted`: IDs de request para fronteras web.                                                                                                                                                                                                                                                                                                                                                                              | El slice que cree la operación añade eventos y failure-path tests; no se guardan prompts completos, headers ni documentos privados.                                                                                                                                                                                                                                   |
...
El punto 2 tiene una única implementación: `getEgressClient()` en
[`src/server/egress/`](../../src/server/egress/). Un adaptador construye el path de
su recurso y nada más; no hay una función que acepte una URL sin fuente, y agregar
una sería reabrir `TM-08` ([ADR 0009](../architecture/adr/0009-egress-boundary.md)).
El punto 1 sigue siendo un control separado: estar en la allowlist de egress da
alcanzabilidad, no derecho a ingerir.
