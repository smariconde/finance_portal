...
| `TM-08` | Una URL, redirect o DNS controlado alcanza metadata, localhost o red privada. SSRF.                                      | high      | `implemented` para la red: allowlist por fuente que empareja host y prefijo de path, HTTPS forzado, resolución validada y fijada al socket, cada redirect re-autorizado, rangos privados y metadata bloqueados, techo de bytes y deadline ([ADR 0009](../architecture/adr/0009-egress-boundary.md)).                                                                                            | El ritmo por corrida —2 requests/s, de a una y 1.000 por corrida— llegó con el provider de la SEC en `F2-03` (`egress-fetch.ts`). Lease, reanudación y concurrencia entre procesos siguen en `F2-05`: hasta entonces la ingesta es un job manual, no un refresh programado.     |
