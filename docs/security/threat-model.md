...
| `TM-08` | Una URL, redirect o DNS controlado alcanza metadata, localhost o red privada. SSRF.                                      | high      | `implemented` para la red: allowlist por fuente que empareja host y prefijo de path, HTTPS forzado, resolución validada y fijada al socket, cada redirect re-autorizado, rangos privados y metadata bloqueados, techo de bytes y deadline ([ADR 0009](../architecture/adr/0009-egress-boundary.md)). | Falta el ritmo: espaciado, concurrencia y presupuesto por corrida (`TM-10`, `TM-11`) llegan con el provider en `F2-03` y el backfill en `F2-05`. Hasta entonces el egress es para llamadas puntuales, no para un job.                                                           |
