...
| Fase 1 - Vertical slice demo | in_progress | `F1-04` cerrado el 2026-08-24 con identidad, provenance y consulta point-in-time sobre una empresa fixture; sigue `F1-05` |
...
**Alcance de `F1-04`:** una unica empresa sintetica recorre identidad completa, publicacion con provenance y consulta point-in-time. El grafo de identidad es una fixture versionada, no una tabla: persistir identidad y corporate actions corresponde a `F2-02`. Ninguna fuente real quedo aprobada, ninguna ruta abre red y la UI todavia no expone estas superficies: eso corresponde a `F1-06`. `F1-05` es el proximo slice autorizado.
