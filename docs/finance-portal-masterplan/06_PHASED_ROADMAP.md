...
| Fase 1 - Vertical slice demo | in_progress | `F1-06` entregado el 2026-08-25 con `/valuacion/demo`; queda abierto solo por la revision renderizada desktop/mobile |
...
**Alcance de `F1-06`:** `/valuacion/demo` publica la corrida demo del motor `fcff_base` con toda su evidencia: contrato point-in-time, hechos reportados con freshness versionada, ausencias declaradas con motivo, supuestos separados de los hechos, proyeccion y descuento, puente EV-equity, sensibilidad WACC/g accesible, transformaciones aplicadas y policy checks. La empresa es sintetica y se declara como tal; no hay recomendacion, precio objetivo, dato live ni persistencia personal simulada. Los escenarios bear/base/bull siguen siendo Fase 4 y la superficie los nombra como planificados. El render no abre reloj, red ni base, asi que la ruta prerenderiza como estatica y su contenido es identico en demo y en personal.
...
**Bloqueos actuales:** `F1-06` no puede cerrar hasta ejecutar la revision renderizada desktop/mobile a 1440x900 y 390x844 en tema claro y oscuro. No hay navegador disponible en la sesion y los scripts `live` de Impeccable no estan aprobados por `AGENTS.md`; el chequeo estatico sobre el HTML prerenderizado no se presenta como sustituto.
